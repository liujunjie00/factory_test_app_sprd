...
/* 	public static final String ENG_MMI2TEST_TABLE = "mmi2test";
...
//    private static Sqlite mEngSqlite;

 /*    public static synchronized Sqlite getInstance(Context context) {
        if (mEngSqlite == null) {
            mEngSqlite = new Sqlite(context);
        }
        return mEngSqlite;
    } */
    ...
    protected Sqlite(Context context){
        ...
        try {
            ...
            Log.v(TAG, "Sqlite mSqLiteDatabase= " + mSqLiteDatabase);
 ...
 public ArrayList<TestItem> queryData(ArrayList<TestItem> queryListitem) {
	...
	public void deleteDB() {
         ...
         //   if(mSqLiteDatabase.isOpen()){
                mSqLiteDatabase.execSQL("DELETE FROM " + ENG_STRING2INT_TABLE +";");
                mSqLiteDatabase.execSQL("update splite_sequence set seq=0 where name ="+"\'"+ ENG_STRING2INT_TABLE + "\'"+";");
         //   } 
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public int querySystemFailCount() {
		...
		/* 	///////////table2
            ...
            ArrayList<TestItem> supportArray2 = MMI2TestItemList.getInstance(
                    mContext).getTestItemList();

            for (int index = 0; index < supportArray2.size(); index++) {
                ContentValues cv = new ContentValues();
                cv.put(ENG_MMI2TEST_NAME, supportArray2.get(index)
                        .getTestClassName());
                cv.put(ENG_MMI2TEST_DISPLAYNAME, supportArray2.get(index)
                        .getTestName());
