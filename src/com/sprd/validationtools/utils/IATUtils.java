...
public class IATUtils {
    ...
    private static String getAtCmdAidl(String cmd, int phoneId) {
        return String.format("sendAt %d %s", phoneId, cmd);
    }

    public static synchronized String sendATCmd(String cmd, int phoneId) {
        String res = SocketUtils.sendCmdByAidl(AT_SOCKET_NAME, getAtCmdAidl(cmd, phoneId));
        if(res != null) {
            return res;
        }
