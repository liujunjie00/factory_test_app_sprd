...
public class SecondaryCameraTestActivity extends BaseActivity implements
    ...
    private static final String TAKE_IMAGE_NAME = "sec_backphoto.jpg";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String type = SystemPropertiesProxy.get("persist.sys.cam3.type", "unknown");
        Log.d(TAG, "onCreate cam3.type=" + type);

        if("back_blur".equals(type)) {
            mCameraID = SystemPropertiesProxy.get("persist.sys.cam3.multi.cam.id", "2");
    ...
    protected void onResume() {
        if("back_sbs".equals(mCam3Type)){
            SystemPropertiesProxy.set("persist.camera.sbs.mode","slave");
            SystemPropertiesProxy.set("persist.sys.camera.raw.mode","raw");
        }
    ...
    protected void onPause() {
        if(mCam3Type != null && mCam3Type.equals("back_sbs")){
            SystemPropertiesProxy.set("persist.camera.sbs.mode","0");
            SystemPropertiesProxy.set("persist.sys.camera.raw.mode","0");
        }
        closeCamera();
        super.onPause();
    }
    ...
    public static boolean isSupport(Context context) {
        ...
        String cam3type = SystemPropertiesProxy.get("persist.sys.cam3.type", "unknown");
        Log.d(TAG, "SecondaryCameraTestActivity numberOfCameras="
                    + numberOfCameras + ",cam3type=" + cam3type);
        if (numberOfCameras <= 2) {
            return false;
        }
        boolean calibraionSupportFlag = false;
        String calibraionSupport = SystemPropertiesProxy.get("persist.sys.3d.calibraion", "0");
        ...
        if ((cam3type.equals("back_blur") || cam3type.equals("back_sbs")) || calibraionSupportFlag) {
            return true;
        } else {
            return false;
        }
