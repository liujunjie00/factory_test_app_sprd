...
public abstract class TestItemList {
    ...
    private ArrayList<TestItem> mAutoTestItemList = new ArrayList<TestItem>();

    public TestItemList(Context context) {
        addAndFilterTestItemList(context, getfilterClassName());
        addAndFilterAutoTestItemList(context, getfilterClassName());
    }
    ...
    private ArrayList<TestItem> addAndFilterTestItemList(Context context,
        ...
        for (String testItemClassName : filterClassName) {
            ...
            boolean isSupport = Const.getSupportInfo(context, testClassName_);
    ...
    public ArrayList<TestItem> getAutoTestItemList() {
        synchronized (mAutoTestItemList) {
            return mAutoTestItemList;
        }
    }

    private ArrayList<TestItem> addAndFilterAutoTestItemList(Context context,
            final String[] filterClassName) {
        if (filterClassName == null) {
            return null;
        }
        for (String testItemClassName : filterClassName) {
            ...
            TestItem iTestItem = new TestItem(testName_, testPackageName_,
                    testClassName_, testResult_, labelRes);
            mAutoTestItemList.add(iTestItem);
        }
        return mAutoTestItemList;
    }
