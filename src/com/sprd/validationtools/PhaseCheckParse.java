...
public class PhaseCheckParse {
	...
	private static int TYPE_CHECK_KEYBOX = 70;   //hzx add for read keybox status
...
Log.d(TAG, "tqy  +++ station = " + station);			
    ...
    public int getKeyboxFlag(){ //hzx add for read keybox status
        int retValue = 0;
        try{
            Parcel data = new Parcel();
            Parcel reply = new Parcel();
            binder.transact(TYPE_CHECK_KEYBOX, data, reply, 0);
            retValue = reply.readInt();
            Log.d(TAG, "get keybox SUCESS, retValue=:" + retValue);
            data.recycle();
        }catch (Exception ex) {
            Log.e(TAG, "Exception " + ex.getMessage());
            retValue = 0;
    ...
    public String getChargeChargingstatus(int type) {
        String result = null;
        try {
            Parcel data = new Parcel();
            Parcel reply = new Parcel();
            binder.transact(type, data, reply, 0);
            result = reply.readString();
            Log.d(TAG, "get getChargeChargingstatus retValue is : " + result);
            data.recycle();
        } catch (NullPointerException | IndexOutOfBoundsException | UnsupportedEncodingException ex) {
            Log.e(TAG, "Exception " + ex.getMessage());
